...
async function initViewer() {
  ...
  const apiInitialized = await ViewerCore.initializeAgentsAPI();
  if (apiInitialized) {
...
async function handleGenerateOutput() {
  ...
  try {
    ...
    if (!ViewerCore.agentsAPI) {
      ...
      ViewerCore.agentsAPI = new AgentsAPI(apiKey, baseURL, model);
    }
    
    // Send the prompt to the AI
    const response = await ViewerCore.agentsAPI.sendMessage(prompt);
...
async function handleSendMessage() {
  ...
  if (message) {
    ...
    if (ViewerCore.agentsAPI) {
      ...
      // Make API call
      const response = await ViewerCore.agentsAPI.sendMessage(message);
...
function handleSaveApiSettings() {
  ...
  const baseUrl = apiBaseUrl || ViewerCore.envVars.OPENAI_API_BASE || 'https://api.openai.com/v1';
  const model = modelName || ViewerCore.envVars.MODEL_NAME || 'gpt-4o';
